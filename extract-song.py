#Creating Gui#importing required libraries
...
window = Tk()
window.geometry("600x600") #add size
window.title("Lyrics Extractor") #add title
head = Label(window, text="Enter the song you want lyrics for", font=("Poppins 15"))
head.pack(pady=20)
Entry(window, textvariable=song).pack()
Message(window, textvariable=result, bg="khaki").pack(side="top", anchor=W,fill=BOTH, expand=1)

#Create a button
Button(window,text="GO",command=get_lyrics).pack()

def get_lyrics():
    song_name = "AIzaSyAcZ6KgA7pCIa_uf8-bYdWR85vx6-dWqDg"
    api_key = "AIzaSyAcZ6KgA7pCIa_uf8-bYdWR85vx6-dWqDg"
    engine_id = "aa2313d6c88d1bf22"
    extract_lyrics = SongLyrics(api_key, engine_id)# going to the website for which we have got the engine id
    song_lyrics = extract_lyrics.get_lyrics(song_name)#getting the lyrics
